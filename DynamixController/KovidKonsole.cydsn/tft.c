...
void write8_a0(uint8 data)
{
    ...
    SPIM_TFT_WriteTxData(data);         		        // send data to transmit buffer
    while (!(SPIM_TFT_ReadTxStatus() & 0x01)){};	    // wait for data to be sent
}
...
void writeM8_a1(uint8 *pData, int N)
{
    ...
    for (i=0; i<N; i++)
    {
        SPIM_TFT_WriteTxData(pData[i]);               // send data to transmit buffer
        while (!(SPIM_TFT_ReadTxStatus() & 0x01)){};  // wait for data to be sent
    }
}
