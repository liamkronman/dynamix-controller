/*******************************************************************************
* File Name: SPIM_Audio_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    SPIM_Audio_IntClock_CLKEN |= SPIM_Audio_IntClock_CLKEN_MASK;
	SPIM_Audio_IntClock_CLKSTBY |= SPIM_Audio_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    SPIM_Audio_IntClock_CLKEN &= (uint8)(~SPIM_Audio_IntClock_CLKEN_MASK);
	SPIM_Audio_IntClock_CLKSTBY &= (uint8)(~SPIM_Audio_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_StopBlock(void) 
{
    if ((SPIM_Audio_IntClock_CLKEN & SPIM_Audio_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(SPIM_Audio_IntClock__CFG3)
        CLK_DIST_AMASK = SPIM_Audio_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SPIM_Audio_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SPIM_Audio_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SPIM_Audio_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SPIM_Audio_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        SPIM_Audio_IntClock_CLKSTBY &= (uint8)(~SPIM_Audio_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = SPIM_Audio_IntClock_GetSourceRegister();
    uint16 oldDivider = SPIM_Audio_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SPIM_Audio_IntClock_CLKEN & SPIM_Audio_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SPIM_Audio_IntClock_DIV_PTR, clkDivider);
                SPIM_Audio_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SPIM_Audio_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SPIM_Audio_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SPIM_Audio_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(SPIM_Audio_IntClock_DIV_PTR, clkDivider);
				SPIM_Audio_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 SPIM_Audio_IntClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(SPIM_Audio_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    SPIM_Audio_IntClock_MOD_SRC |= modeBitMask & (uint8)SPIM_Audio_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    SPIM_Audio_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SPIM_Audio_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SPIM_Audio_IntClock_GetModeRegister(void) 
{
    return SPIM_Audio_IntClock_MOD_SRC & (uint8)(SPIM_Audio_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = SPIM_Audio_IntClock_GetDividerRegister();
    uint8 oldSrc = SPIM_Audio_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SPIM_Audio_IntClock_MOD_SRC |= CYCLK_SSS;
        SPIM_Audio_IntClock_MOD_SRC =
            (SPIM_Audio_IntClock_MOD_SRC & (uint8)(~SPIM_Audio_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SPIM_Audio_IntClock_GetSourceRegister(void) 
{
    return SPIM_Audio_IntClock_MOD_SRC & SPIM_Audio_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SPIM_Audio_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    SPIM_Audio_IntClock_PHASE = clkPhase & SPIM_Audio_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SPIM_Audio_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SPIM_Audio_IntClock_GetPhaseRegister(void) 
{
    return SPIM_Audio_IntClock_PHASE & SPIM_Audio_IntClock_PHASE_MASK;
}
